...
export default class HttpClient {
  ...
  private async request(url: string | URL, init?: RequestInit) {
    try {
      const authorizationHeader: HeadersInit = this.authStore.isAuthorized
        ? [['Authorization', `${this.authStore.token?.tokenType} ${this.authStore.token?.accessToken}`]]
        : []
  ...
  private adjustHeader(body?: object | BodyInit) {
    const headers = new Headers()
  ...
  private adjustBody(body?: object | BodyInit): BodyInit | null {
  ...
  post<T>(url: string, body?: object | BodyInit, query?: Record<string, unknown>): Promise<T> {
    const targetUrl = this.buildUrl(url, query)
    const headers = this.adjustHeader(body)
    const targetBody = this.adjustBody(body)
