					...
					.oidc(oidc ->
						oidc
							.logoutEndpoint(logoutEndpoint ->
								logoutEndpoint
									.logoutRequestConverter(createOidcLogoutAuthenticationConverter(registeredClientRepository))
							)
					)
			)
				...
				if (clientId != null) {
					...
					if (registeredClient != null && !CollectionUtils.isEmpty(registeredClient.getPostLogoutRedirectUris())) {
						...
						if (postLogoutRedirectUri != null) {
							authentication = new OidcLogoutAuthenticationToken(authentication.getIdTokenHint(),
								(Authentication) authentication.getPrincipal(), authentication.getSessionId(),
								authentication.getClientId(), postLogoutRedirectUri, authentication.getState());
						}
					}
				}
			}
