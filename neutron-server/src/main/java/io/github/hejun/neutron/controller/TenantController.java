...
public class TenantController {
	...
	@GetMapping
	public IPage<TenantListVO> findPage(@Valid @Min(value = 1, message = "页码不可小于1") @RequestParam(defaultValue = "1") Long page,
										@Valid @Max(value = 200, message = "每页不可大于200") @RequestParam(defaultValue = "15") Long size) {
		return tenantService.findPage(page, size).convert(tenantConverter::convertToList);
	}
